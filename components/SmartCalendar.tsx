...
const SmartCalendar: React.FC<SmartCalendarProps> = ({ 
  ...
  useEffect(() => {
      if (lastFocusedDate) {
          ...
          if (index !== -1 && dayRefs.current[index]) {
               // Small timeout to ensure DOM is ready and modal is closed
               setTimeout(() => {
  ...
  const handleKeyDown = (e: React.KeyboardEvent, index: number, date: string, record: AttendanceRecord | undefined) => {
      ...
      // RTL Logic: Left Arrow goes to Next Day (index + 1), Right Arrow goes to Prev Day (index - 1)
      switch (e.key) {
          case 'ArrowLeft':
              nextIndex = index + 1;
              break;
          case 'ArrowRight':
              ...
              break;
          case 'ArrowDown':
              nextIndex = index + 4; // Approx grid columns (adjust based on responsive if needed, usually 4 on large screens)
              break;
          case 'ArrowUp':
              nextIndex = index - 4;
              break;
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {dates.map((date, idx) => {
          ...
          // Pass 'employee' to stats calculation

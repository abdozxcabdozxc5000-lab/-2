...
const Reports: React.FC<ReportsProps> = ({ employees, attendanceRecords, config, currentUserRole, currentEmployeeId }) => {
  ...
  // Filter employees passed to calculateRanking based on permission
  ...
  const canViewAll = Permissions.canViewAllReports(currentUserRole) || Permissions.isOwner(currentUserRole) || currentUserRole === 'office_manager';
  
  // If canViewAll is true, it means they can view *their authorized scope*, which is filtered above for Office Manager.
  ...
  const handleDownloadCSV = () => {
      ...
      const rows = displayData.map(row => [
          row.rank,
          `"${row.name}"`, // وضع علامات تنصيص للأسماء لتجنب مشاكل الفواصل
          `"${row.position}"`,
          row.score,
          row.totalDelay,
      ...
      // تجميع الملف مع إضافة BOM لدعم اللغة العربية في Excel
      const csvContent = "\uFEFF" + 
